...
MangaNeloComPlugin.prototype.getInfos = function(url, doc) {
    ...
    if (parts.length < 1) {
        ...
        // manga page
        var elem = doc.querySelector(".panel-story-info>.story-info-right>h1");
        if (!elem) {
            // TODO: add log
    ...
    // chapter page
    var elems = cloneArray(doc.querySelectorAll(".panel-breadcrumb > a.a-h"));
    if (elems.length < 3) {
        // TODO: add log
        console.log("Cannot get manga home page nor its name");
        return null;
    }
    var name = elems[1].innerText;
    var homeUrl = elems[1].getAttribute("href").split("manganelo.com")[1];
    var parts = url.split("/");
